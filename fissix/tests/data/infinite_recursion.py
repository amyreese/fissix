...
bio_st._fields_ = [
    ...
    (
        "callback",
        CFUNCTYPE(c_long, POINTER(bio_st), c_int, STRING, c_int, c_long, c_long),
    ),
    ("cb_arg", STRING),
    ("init", c_int),
    ("shutdown", c_int),
...
BIO_dummy = bio_st
CRYPTO_EX_new = CFUNCTYPE(
    c_int, c_void_p, c_void_p, POINTER(CRYPTO_EX_DATA), c_int, c_long, c_void_p
)
CRYPTO_EX_free = CFUNCTYPE(
    None, c_void_p, c_void_p, POINTER(CRYPTO_EX_DATA), c_int, c_long, c_void_p
)
CRYPTO_EX_dup = CFUNCTYPE(
    c_int,
    POINTER(CRYPTO_EX_DATA),
    POINTER(CRYPTO_EX_DATA),
    c_void_p,
    c_int,
    c_long,
...
dsa_method._fields_ = [
    ...
    (
        "dsa_do_verify",
        CFUNCTYPE(c_int, POINTER(c_ubyte), c_int, POINTER(DSA_SIG), POINTER(DSA)),
    ),
    (
        "dsa_mod_exp",
        CFUNCTYPE(
...
evp_cipher_st._fields_ = [
    ...
    (
        "init",
        CFUNCTYPE(
            c_int, POINTER(EVP_CIPHER_CTX), POINTER(c_ubyte), POINTER(c_ubyte), c_int
        ),
    ),
    (
        "do_cipher",
        CFUNCTYPE(
            c_int, POINTER(EVP_CIPHER_CTX), POINTER(c_ubyte), POINTER(c_ubyte), c_uint
        ),
    ),
    ...
    (
        "set_asn1_parameters",
        CFUNCTYPE(c_int, POINTER(EVP_CIPHER_CTX), POINTER(ASN1_TYPE)),
    ),
    (
        "get_asn1_parameters",
        CFUNCTYPE(c_int, POINTER(EVP_CIPHER_CTX), POINTER(ASN1_TYPE)),
...
rsa_meth_st._fields_ = [
    ...
    (
        "rsa_pub_enc",
        CFUNCTYPE(
            c_int, c_int, POINTER(c_ubyte), POINTER(c_ubyte), POINTER(RSA), c_int
        ),
    ),
    (
        "rsa_pub_dec",
...
ssl_method_st._fields_ = [
    ...
    (
        ...
        CFUNCTYPE(c_long, POINTER(SSL_CTX), c_int, CFUNCTYPE(None)),
    ),
]
...
ssl_ctx_st._fields_ = [
    ...
    (
        "client_cert_cb",
        CFUNCTYPE(
            c_int, POINTER(SSL), POINTER(POINTER(X509)), POINTER(POINTER(EVP_PKEY))
        ),
    ),
    ("ex_data", CRYPTO_EX_DATA),
    ...
    (
        "msg_callback",
        CFUNCTYPE(None, c_int, c_int, c_int, c_void_p, c_ulong, POINTER(SSL), c_void_p),
    ),
...
x509_lookup_method_st._fields_ = [
    ...
    (
        "ctrl",
        CFUNCTYPE(c_int, POINTER(X509_LOOKUP), c_int, STRING, c_long, POINTER(STRING)),
    ),
    (
        "get_by_subject",
        CFUNCTYPE(
            c_int, POINTER(X509_LOOKUP), c_int, POINTER(X509_NAME), POINTER(X509_OBJECT)
        ),
    ),
    (
    ...
    (
        "get_by_alias",
        CFUNCTYPE(
            c_int, POINTER(X509_LOOKUP), c_int, STRING, c_int, POINTER(X509_OBJECT)
        ),
    ),
]
...
x509_store_st._fields_ = [
    ...
    (
        "get_issuer",
        CFUNCTYPE(
            c_int, POINTER(POINTER(X509)), POINTER(X509_STORE_CTX), POINTER(X509)
        ),
    ),
    (
        "check_issued",
        CFUNCTYPE(c_int, POINTER(X509_STORE_CTX), POINTER(X509), POINTER(X509)),
    ),
    ("check_revocation", CFUNCTYPE(c_int, POINTER(X509_STORE_CTX))),
    (
        "get_crl",
        CFUNCTYPE(
            c_int, POINTER(X509_STORE_CTX), POINTER(POINTER(X509_CRL)), POINTER(X509)
        ),
    ),
    ("check_crl", CFUNCTYPE(c_int, POINTER(X509_STORE_CTX), POINTER(X509_CRL))),
    (
        "cert_crl",
        CFUNCTYPE(c_int, POINTER(X509_STORE_CTX), POINTER(X509_CRL), POINTER(X509)),
    ),
